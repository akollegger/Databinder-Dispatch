...
object Neo4jSpec extends Specification {
  ...
  val localNeo4j = Neo4j() // these tests expect Neo4jDB to be running at 127.0.0.1 on port 7474
  val UrlPattern = """^http:\/\/[\w\.]+(:\d+)?(/.*)?$"""

  "A Neo4j server" should {

    doFirst { /* start a Neo4j test server */ }

    doLast { /* stop the Neo4j test server */ }
